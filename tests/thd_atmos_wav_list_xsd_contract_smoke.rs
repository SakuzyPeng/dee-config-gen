...
use dee_config_gen::template::thd_atmos_wav_list_v1::params::PARAM_SCHEMAS;

use common::{
    EvidenceTier, THD_ATMOS_WAV_LIST_XSD_CONTRACT_PATH, evidence_tier_for_param,
    find_filter_param_path_with_prefix, load_xsd_contract_at,
};

fn thd_atmos_wav_list_filter_param_path<'a>(
...
#[test]
fn thd_atmos_wav_list_xsd_contract_json_has_expected_sections() {
    let contract = load_xsd_contract_at(THD_ATMOS_WAV_LIST_XSD_CONTRACT_PATH);

    assert_eq!(contract.meta.template_id, "thd_atmos_wav_list_v1");
...
#[test]
fn thd_atmos_wav_list_xsd_contract_covers_dolby_official_paths() {
    let contract = load_xsd_contract_at(THD_ATMOS_WAV_LIST_XSD_CONTRACT_PATH);

    for schema in PARAM_SCHEMAS {
        if evidence_tier_for_param(schema.key, schema.sources) == EvidenceTier::Official {
            let path = thd_atmos_wav_list_filter_param_path(&contract, schema.key);
            assert!(
                path.is_some(),
                "missing xsd path for official thd_atmos_wav_list param_key={} in contract.paths",
                schema.key
            );
        }
    }
    ...
    for required in [
        "/job_config/input/audio/atmos_mezz/file_name",
        "/job_config/input/audio/atmos_mezz/timecode_frame_rate",
        "/job_config/input/audio/atmos_mezz/offset",
        "/job_config/input/audio/atmos_mezz/ffoa",
