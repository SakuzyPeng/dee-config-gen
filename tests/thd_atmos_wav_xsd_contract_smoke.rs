...
fn thd_atmos_wav_filter_param_path<'a>(
...
#[test]
fn thd_atmos_wav_xsd_contract_json_has_expected_sections() {
    let contract = load_xsd_contract_at(THD_ATMOS_WAV_XSD_CONTRACT_PATH);

    assert_eq!(contract.meta.template_id, "thd_atmos_wav_v1");
...
fn thd_atmos_wav_xsd_contract_covers_dolby_official_paths() {
    ...
    for schema in param_schemas("thd_atmos_wav_v1").unwrap() {
        if evidence_tier_for_param(schema.key, schema.sources) == EvidenceTier::Official {
            ...
            assert!(
                path.is_some(),
                "missing xsd path for official thd_atmos_wav param_key={} in contract.paths",
                schema.key
            );
        }
    }
    ...
    for required in [
        ...
        "/job_config/input/audio/wav/file_name",
        "/job_config/input/audio/wav/timecode_frame_rate",
        "/job_config/input/audio/wav/offset",
        "/job_config/input/audio/wav/ffoa",
    ] {
        assert!(
