...
- current header version macros are fixed to:
...
Status-code compatibility contract (v1):
- numeric values `0..6` are frozen: no reorder, no reuse, no semantic reassignment
- stage mapping is fixed: `parse -> PARSE_ERROR`, `resolve/validate -> RESOLVE_ERROR`, `render -> RENDER_ERROR`
- `PANIC` is reserved for `catch_unwind` boundary capture and must not be reused for normal business failures
- future error code additions require an ABI version bump, not v1 remapping
...
Caller-side version branching guidance:
- check `DCG_FFI_ABI_VERSION` first and route mismatches to a safe fallback path
- optionally check `DCG_FFI_HEADER_VERSION` for finer header-level feature gates
- keep a `default` branch in status-code switches to handle future unknown values safely
...
- Release trigger: push a `v*` tag (for example `v0.1.2`)
