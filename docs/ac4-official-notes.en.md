...
- A native Windows rerun on 2026-03-19 reconfirmed Atmos-lane success for both direct `output/ac4` and direct `output/mp4`, and manual `mp4muxer` from the generated `.ac4` also succeeded. Isolated serial reruns with dedicated `STATE_DIR` also reproduced direct MP4 success; earlier `Access violation` reports are now treated as shared-state/parallel-run artifacts rather than template rendering failure.
