...
pub use filter::ThdAtmosWavListV1Filter;

pub const THD_ATMOS_WAV_LIST_V1: ThdAtmosWavListV1 = ThdAtmosWavListV1;

pub struct ThdAtmosWavListV1;

impl Template for ThdAtmosWavListV1 {
    ...
    fn defaults(&self, profile: Profile, encode_mode: EncodeMode) -> ResolvedFilter {
        ResolvedFilter::ThdAtmosWavListV1(defaults::defaults(profile, encode_mode))
    }
    ...
    fn apply_overrides(
        ...
        thd_wav_list_v1::reject_unsupported_overrides(overrides)?;
        ...
        if let Some(v) = &overrides.starting_timecode {
            ...
            filter.starting_timecode = thd_wav_list_v1::validate_thd_starting_timecode(&value)?;
        }
        ...
        if let Some(v) = &overrides.start {
            ...
            filter.start = thd_wav_list_v1::validate_thd_boundary_timecode("start", &value)?;
        }
        if let Some(v) = &overrides.end {
            let value = validate_string_param("end", v, &ctx)?;
            filter.end = thd_wav_list_v1::validate_thd_boundary_timecode("end", &value)?;
        }
        ...
        if let Some(v) = &overrides.prepend_silence_duration {
            ...
            filter.prepend_silence_duration =
                thd_wav_list_v1::validate_thd_silence_duration("prepend_silence_duration", &value)?;
        }
        if let Some(v) = &overrides.append_silence_duration {
            ...
            filter.append_silence_duration =
                thd_wav_list_v1::validate_thd_silence_duration("append_silence_duration", &value)?;
        }
    ...
    fn validate_runtime_compatibility(
        ...
        let filter = as_filter(filter);
        if input_media.len() < 3 {
            bail!(
                "template_id 'thd_atmos_wav_list_v1' requires one atmos_mezz input and 2, 6 or 8 wav_list stems"
            );
        }
        let (atmos_mezz, wav_list) = input_media
            .split_first()
...
fn as_filter(filter: &ResolvedFilter) -> &ThdAtmosWavListV1Filter {
    match filter {
        ResolvedFilter::ThdAtmosWavListV1(value) => value,
        _ => panic!("thd_atmos_wav_list_v1 received wrong ResolvedFilter variant"),
    }
}

fn as_filter_mut(filter: &mut ResolvedFilter) -> Result<&mut ThdAtmosWavListV1Filter> {
    match filter {
        ResolvedFilter::ThdAtmosWavListV1(value) => Ok(value),
        _ => bail!("thd_atmos_wav_list_v1 received wrong ResolvedFilter variant"),
    }
}
