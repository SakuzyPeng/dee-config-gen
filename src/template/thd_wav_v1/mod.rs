...
fn as_filter(filter: &ResolvedFilter) -> &ThdWavV1Filter {
    match filter {
        ...
        ResolvedFilter::Ac4V1(_) => panic!("thd_wav_v1 received wrong ResolvedFilter variant"),
...
fn as_filter_mut(filter: &mut ResolvedFilter) -> Result<&mut ThdWavV1Filter> {
    match filter {
        ...
        ResolvedFilter::Ac4V1(_) => bail!("thd_wav_v1 received wrong ResolvedFilter variant"),
